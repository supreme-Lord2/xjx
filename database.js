...
// Read database — with automatic backup recovery if the primary file is corrupt
  ...
  try {
    if (fs.existsSync(bakPath)) {
      ...
      if (bak.trim()) {
        ...
        // Restore the primary so future reads work normally
...
// Write database — atomic: write to a temp file then rename so a mid-write
// kill can never leave the JSON file truncated or corrupted.
const writeDB = (filePath, data) => {
  try {
    ...
    // 2. Promote previous primary to .bak (best-effort — ignore errors)
    try {
