...
const handleGroupUpdate = async (sock, update) => {
  try {
    ...
    if (action === 'demote' || action === 'promote') {
      try {
        console.log(`[AntiSecurity] action=${action} | group=${id} | author=${actor} | participants=${JSON.stringify(participants)}`);

        const antidemoteCmd  = commands.get('antidemote');
        const antipromoteCmd = commands.get('antipromote');

        console.log(`[AntiSecurity] antidemoteCmd=${!!antidemoteCmd} handleDemote=${!!antidemoteCmd?.handleDemote} | antipromoteCmd=${!!antipromoteCmd} handlePromote=${!!antipromoteCmd?.handlePromote}`);
