...
// View-once message store — keyed by message ID, auto-purged after 30 minutes
...
const handleMessage = async (sock, msg) => {
  try {
    ...
    // ── View-once emoji-reaction reveal ──────────────────────────────────────
    // If someone reacts with any emoji to a view-once message, send the
    // media privately to the reactor's DM with the bot.
    if (msg.message?.reactionMessage) {
      ...
      // ── 2. Bot reacts back with an emoji ────────────────────────────────────
      // Only react when it's not the bot's own reaction and emoji is non-empty
      if (emoji && !msg.key.fromMe) {
        try {
          const replyEmojis = ['❤️','🔥','😂','👏','🎉','💯','✨','😍','🤩','💪','👌','🙌'];
          const pick = replyEmojis[Math.floor(Math.random() * replyEmojis.length)];

          // React to the REACTION message itself (the one that just arrived)
