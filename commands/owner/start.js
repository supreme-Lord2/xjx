...
async function extractZip(zipPath, outDir) {
    ...
    // Try using adm-zip (pure JS, no external dependencies)
    ...
    try {
        ...
        // If adm-zip is not installed, attempt to install it once
        if (!admZipAvailable) {
            try {
            ...
            // adm-zip is installed but extraction failed – fall through
        }
    }
...
function cleanDirectory(dir) {
    ...
    for (const entry of fs.readdirSync(dir)) {
        ...
        try {
            ...
            // silent fail – file might be locked or already gone
        }
    }
}
...
module.exports = {
    name: 'update',              // primary command name
    aliases: ['start'],          // keep old alias for compatibility
    category: 'owner',
    ...
    async execute(sock, msg, args, extra) {
        ...
        try {
            ...
            // Verify the extracted content is not empty
            ...
            // The ZIP may contain a single top‑level folder (like a GitHub repo archive)
            ...
            // Optional: run npm install if package.json changed (uncomment if needed)
            ...
            // Attempt graceful restart via PM2, else exit
