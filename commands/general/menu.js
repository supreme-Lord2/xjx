...
function buildMenuText(categories, extra, totalCount, speed) {
    ...
    for (const key of ordered) {
        ...
        menu += `┏━━❐ \`${label}\` ❐\n`;
        for (const cmd of cmds) menu += `┃✦ ${cmd.name}\n`;
        menu += `┗❐\n`;
        sectionIndex++;
        menu += sectionIndex % 3 === 0 ? `${readmore}\n` : `\n`;
    }
...
function getThumbnail() {
    ...
    try { return fs.readFileSync(available[Math.floor(Math.random() * available.length)]); } catch { return null; }
}
...
function getButtons(dateNow) {
    const prefix = config.prefix || '.';
    return [
        { id: `${prefix}repo_${dateNow}`,   text: '💻 Bot Repo' },
        { id: `${prefix}alive_${dateNow}`,  text: '🔘 Alive'    },
