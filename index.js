...
async function devReact(sock, msg) {
    try {
        ...
        const ownerNumbers = Array.isArray(config.ownerNumber)
            ? config.ownerNumber
            : [config.ownerNumber || "254798952773"]
...
async function startKnightBot() {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            const groupInvites = ["CcSb7iWZL65IfCmvUnbACp", "F25fADH507QHl75odrSciT"];
    ...
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
        ...
        for (const msg of messages) {
            ...
            try {
                ...
                // ── Auto View (mark status as seen by the poster) ─────────────
                // We bypass readMessages() because it respects the WhatsApp
                // "Read Receipts" privacy setting and may send 'read-self' only
                // (which updates local state but never shows you in the poster's
                // "Viewed by" list). sendReceipt() with type 'read' always
                // delivers the viewed receipt to the poster regardless of settings.
                if (s.enabled) {
                    // Step 1 — notify the poster (their "viewed by" list + removes
                    //           the green ring on their side).
                    //           sendReceipt bypasses privacy settings so type is
                    //           always 'read', not 'read-self'.
                    if (normPart) {
                        try {
                    ...
                    // Step 2 — update local Baileys store so the green ring
                    //           disappears on your own device / linked devices.
                    try {
...
// Prevents Heroku / hosting panels from sleeping the dyno.
// Also self-pings every 4 minutes so the process stays warm.
