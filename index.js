        ...
        for (const msg of messages) {
            ...
            try {
                ...
                if (s.enabled && normPart) {
                    // 1. Mark the message as read locally
                    try {
                        await sock.readMessages([msg.key])
                    } catch (_) {}
                    // 2. Send the read receipt directly to the poster's JID so
                    //    WhatsApp registers the view and greyes their status circle
                    await new Promise(r => setTimeout(r, 500 + Math.floor(Math.random() * 500)))
                    try {
                        await sock.sendReceipt(normPart, undefined, [msg.key.id], 'read')
                    } catch (_) {}
                }
